...
void vulnerable(int fd_read, int fd_write) {
    ...
    */
    ...
    // target 7 is target 6 without the secret cookie, so just echo the data
    // received from the child process to stdout regardless of the first four
    // bytes
    // uncomment this for a little more of a challenge
    /* printf("Received:\n"); */
